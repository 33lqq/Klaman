function [xupd_3, Pupd_3, R_hat3, RMS_3, RMS_3_1, SRNFN_P3, SRNFN_R3] = VBAKF_R(A,Bd,C,Q,R,Q_cons,R_cons,R_true,Xtrue,Ytrue,Pupd,N,mc, RMS_3, RMS_3_1, SRNFN_P3, SRNFN_R3)
% parameter;
nx = size(Xtrue,1);
ny = size(Ytrue,1);
...
%% ini, C , L+1
uupd_3((round(N/3)+1)) = 5; %3
Uupd_3(:,:,(round(N/3)+1)) = diag(R(:,3));

R_hat_3(:,:,(round(N/3)+1)) = diag(R(:,3));
for k = (round(N/3)+2):(2*round(N/3)+1) %epoch k
    xpre_3(:,:,k) = A * xupd_3(:,:,k-1);
    Ppre_3(:,:,k) = A * Pupd_3(:,:,k-1) *A' + Bd*Q_cons(:,k-1)*Bd';
...
%% ini, E , 2*L+1
uupd_3((2*round(N/3)+1)) = 5; %3
Uupd_3(:,:,(2*round(N/3)+1)) = diag(R(:,1));

R_hat_3(:,:,(2*round(N/3)+1)) = diag(R(:,1));
for k = (2*round(N/3)+2):(3*round(N/3)+1) %epoch k
    xpre_3(:,:,k) = A * xupd_3(:,:,k-1);
    Ppre_3(:,:,k) = A * Pupd_3(:,:,k-1) *A' + Bd*Q_cons(:,k-1)*Bd';
