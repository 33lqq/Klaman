function [xupd_4, Pupd_4,R_hat4, R_hat41, RMS_4, RMS_4_1, RMS_41, RMS_41_1, SRNFN_P4, SRNFN_P41, SRNFN_R4,SRNFN_R41] = My_filter(A,Bd,C,Q,R,Q_cons,R_cons,R_true,Xtrue,Ytrue,Pupd,Ns,mc, RMS_4, RMS_4_1, RMS_41, RMS_41_1, SRNFN_P4, SRNFN_P41, SRNFN_R4,SRNFN_R41)
% parameter;
nx = size(Xtrue,1);
ny = size(Ytrue,1);
...
R_hat_4(:,:,1) = diag(R(:,5));
for k = 2:(round(Ns/3)+1) % epoch 500
...
uupd((round(Ns/3)+1)) = 5; %4
Uupd(:,:,(round(Ns/3)+1)) = diag(R(:,3));

tupd((round(Ns/3)+1)) = 6;
Tupd(:,:,(round(Ns/3)+1)) = A* Pupd_4(:,:,(round(Ns/3)+1)) *A' + Bd*diag(Q(:,3))*Bd';%mvnrnd(eye(4),eye(4)); %eye(4);

R_hat_4(:,:,(round(Ns/3)+1)) = diag(R(:,3));
for k = (round(Ns/3)+2):(2*round(Ns/3)+1) %epoch k
    xpre_4(:,:,k) = A * xupd_4(:,:,k-1);
    Ppre_4(:,:,k) = A * Pupd_4(:,:,k-1) *A' + Bd*Q_cons(:,k-1)*Bd';
...
%% ini, E , 2*L+1
uupd((2*round(Ns/3)+1)) = 5; %4
Uupd(:,:,(2*round(Ns/3)+1)) = diag(R(:,1));

tupd((2*round(Ns/3)+1)) = 6;
Tupd(:,:,(2*round(Ns/3)+1)) = A* Pupd_4(:,:,(2*round(Ns/3)+1)) *A' + Bd*diag(Q(:,1))*Bd';%mvnrnd(eye(4),eye(4)); %eye(4);

R_hat_4(:,:,(2*round(Ns/3)+1)) = diag(R(:,1));
for k = (2*round(Ns/3)+2):(3*round(Ns/3)+1) %epoch k
    xpre_4(:,:,k) = A * xupd_4(:,:,k-1);
    Ppre_4(:,:,k) = A * Pupd_4(:,:,k-1) *A' + Bd*Q_cons(:,k-1)*Bd';
    % variational measurement update
    upre(k) = zou*uupd(k-1) ;
...
%     d(i) = (1-nami)/(1-nami^(i) );
    d(i) = (1-nami)/(1-nami^(i-4) );
%     R_hat_bwd(:,i) = (1-d(i))*R_hat4(:,i) + d(i)*R_hat_bwd(:,i+1);
    R_hat_bwd(:,i) = d(i)*R_hat4(:,i) + (1-d(i))*R_hat_bwd(:,i+1);
end
